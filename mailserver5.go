...
func main() {
     ...
     err = query.Find(bson.M{}).Limit(120).Sort("-date").All(&p)
     if err != nil {
        log.Fatal(err)
        }
     ...
     if p[0].Up == "UPoverZERO" && sumUp > 2.0{

               won ,_:= strconv.ParseFloat(x[int(sumUp -2.0)].Last,64) 
	       ...
	       wons := strconv.FormatFloat(won - wonlast, 'f', 6,64)
               ...
               rate := p[0].Last
               msg := "5mm18bsT6FEXbfgT18Ask3gV2BTEq6k8GeUdx"   
     ...
     if p[0].Up == "DOWNoverZERO" && sumUp > 2.0{
	       won ,_:= strconv.ParseFloat(x[int(sumUp -2.0)].Last,64) 
	       wonlast, _ := strconv.ParseFloat(x[int(sumUp)].Last,64) 
	       wons := strconv.FormatFloat(won - wonlast, 'f', 6,64)
               subject := "BTC/USD rate Will be Decrease!"
               rate := p[0].Last
               msg := "5mm1JTF1QpJ6yNhtF6fRUEM14x6AxBL8F9TyE"   
               body := "Subject:" + subject + "\n" + msg + "\n" + rate +"\n" + count + "ago\n" + wons + "\n" + t
               err := smtp.SendMail(
                   "smtp.gmail.com:587",
                   auth,
